...
class tcpdi_parser {
	...
	//页面与对象映射缓存
    public $page_map_cache=NULL;
	
	//同一文件的其他PDF对象
    public $otherPdfList=NULL;
...
// -----------------------------------------------------------------------------
	...
	protected function decodeXref($startxref, $xref=array()) {
		...
		$xref_end_pos=stripos($this->pdfdata,'trailer',$startxref);
		if($xref_end_pos===false){
			$this->Error('Unable to find Xref Data');
		}
		
		$xref_str=substr($this->pdfdata,$startxref,$xref_end_pos-$startxref);
		//去除字符头部的xref 和尾部的换行符号
		$xref_str=preg_replace('/^xref(\r\n|\r|\n)*|(\r\n|\r|\n)*$/i','',$xref_str);
		$xref_arr=preg_split('/\r\n|\r|\n/',$xref_str);
		
		$xref_pos=0;
		$no_obj_num=0;
		$obj_num=0;
		foreach($xref_arr as $item)
		{
			$last_char=substr($item,-1,1);
			if($last_char=="n")
			{
				$arr_tmp=explode(" ",$item);
				$gen_num = intval($arr_tmp[1]);
				// check if object already exist
				if (!isset($xref['xref'][$xref_pos][$gen_num])) {
					// store object offset position
					$xref['xref'][$xref_pos][$gen_num] = intval($arr_tmp[0]);
				}
				$xref_pos++;
				$obj_num++;
			}
	...
	/**
	 * 判断当前pdf数据是否Outlines书签数据
	 * @return bool
	 */
	public function hasOutlines() {
		$params = $this->getObjectVal($this->xref['trailer'][1]['/Root']);
    	$objref = null;
    	...
    	if ($objref == null || $objref[0] !== PDF_TYPE_OBJREF) {	//如果未找到书签返回false
    		// Offset not found.
    		return false;
    	}
		
		$dict = $this->getObjectVal($objref);
		$has_ol=$dict[1][1]["/Count"][1]>0;
	...
	public function getOutlines() {
		...
		$outlines=array();
		$outline_first=$dict[1][1]["/First"];

		$outline_arr=$this->getOutline($outline_first);
	
		return $outline_arr;
		
	}
	
	function getOutline($obj)
	{	
		$outline_arr=array();
		
		while(true)
		{
			$obj_val=$this->getObjectVal($obj);
		
			$page_pos=$this->getGoto($obj_val[1][1]["/A"]);			
			$title=$obj_val[1][1]["/Title"][1];
			$title=$this->convStr($title);	
			$title=str_replace(chr(13).chr(10),'',$title);
			$pid=$obj_val[1][1]['/Parent'][1];
			$outline_item=array("title"=>$title,"pos"=>$page_pos,"pid"=>$pid);
			
			if(isset($obj_val[1][1]["/First"]))	//如果有子目录则递归调用本函数
			{
				$child=$this->getOutline($obj_val[1][1]['/First']);
				foreach($child as $ckey=>$citem)
				{
					$outline_arr[$ckey]=$citem;
				}
				unset($child);
			}
			
			$outline_arr[$obj_val['obj']]=$outline_item;
			if(isset($obj_val[1][1]["/Next"]))	//如果还有下一个则继续
			{
				$obj=$obj_val[1][1]["/Next"];
			}
			else
			{
				return $outline_arr;
	...
	/*获取页面转跳页数
	  @param array pdf object
	  @return int 
	*/
	function getGoto($obj)
	{
		$page_val=$this->getObjectVal($obj);
		
		$dest_obj=$page_val[1][1]["/D"];
		$dest_obj_val=$this->getObjectVal($dest_obj);
		
		if($this->page_map_cache===NULL)	//如果页面映射为空则初始化
		{
			$this->page_map_cache=$this->getPageMap();
		}
		$dest_obj_num=$dest_obj_val[1][0][1];

		$page_no=isset($this->page_map_cache[$dest_obj_num])?$this->page_map_cache[$dest_obj_num]:1;
		return $page_no;		
	}
	...
	*/
	function getPageMap()
	{
		$page_map_cache=array();
		foreach($this->pages as $key=>$item)
		{
			$page_map_cache[$item["obj"]]=$key+1;
		}
		return $page_map_cache;
	}
	
	/*转换字符串*/
	function convStr($str)
	{
		$arr1=array(chr(92).chr(110),chr(92).chr(114),chr(92).chr(116),chr(92).chr(98),chr(92).chr(102),chr(92).chr(40),chr(92).chr(41),chr(92).chr(100),chr(92).chr(92));
		$arr2=array(chr(10),chr(13),chr(9),chr(8),chr(12),chr(40),chr(41),chr(100),chr(92));
		$str=str_replace($arr1,$arr2,$str);		//解决字符串中有两个 \\
