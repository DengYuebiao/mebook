...
class IndexController extends BaseController
{
	...
	public function login()
	{
		...
		if (isset($_POST['user_name'])) {
			...
			//唯一登录
			M('session')->where("session_id like '".$user_name."%'")->delete();
			session('[destroy]');
			session(array_merge(C('SESSION_OPTIONS'),array('id'=>$user_name.time())));

			$data = array(
			   'last_login'=>time(),
			);
			$mod_user->where("user_id=".$user_info['user_id'])->save($data);
			session('user_info', $user_info);
			$this->success(l('user_login_ok'), u('user/index'));
			return NULL;
		}
		$this->display();
	}
