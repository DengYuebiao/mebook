...
function verifyUser(pkey_id,is_verify,elem)
{
	if(confirm(L('verify_confirm'))==true)
	{
		var h=getSite()+"/Userman/verify/is_verify/"+is_verify+"/user_id/"+pkey_id;
		location.href=h;
	}
...
function getClcItemHtml(row_data)
{
	...
	var row_html='<li><div class="clc_info f-cb"><div class="info"><input type="hidden" class="clc_id" value="'+clc_id+'" /><input type="hidden" class="pid" value="'+pid+'" /><input type="hidden" class="order_num" value="'+order_num+'" /><img class="list_ico" onclick="listChild(this)" src="/Public/image/ico1.png"><span class="clc">'+clc+'</span>&nbsp;<span class="clc_desc">'+clc_desc+'</span></div><div class="btns"><a  onclick="return clcEdit(this)">'+L('edit')+'</a>&nbsp;<a  onclick="return clcAdd(this)">'+L('add')+'</a>&nbsp;<a  onclick="return clcDrop(this)">'+L('drop')+'</a>&nbsp;<a  onclick="return clcAddChlid(this)">'+L('add_child')+'</a>&nbsp;<a onclick="return clcUp(this)">'+L('up')+'</a>&nbsp;<a onclick="return clcDown(this)">'+L('down')+'</a></div></div></li>';
...
{
	 var url=getSite()+"/Book/get_clc_child/clc_id/"+clc_id;	
	 $.ajax({
		 url:url,
		 dataType:"json",
		 async:false,
